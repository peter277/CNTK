...
template <class ElemType>
double LatticeFreeMMINode<ElemType>::CalculateNumerators(const Matrix<ElemType>& labelMatrix, const size_t nf)
{
    if (nf == 0) return 0;
    size_t nsenones = labelMatrix.GetNumRows();
    ...
    double logFordwardScore = log(m_alphaNums[nstates * nf - 1] * m_fsa[m_stateSequence[nstates - 1]][-1].second);
...
#ifdef _DEBUG
    cout << "log forward score: " << logFordwardScore << endl;
    ...
    const Matrix<ElemType>& tmap, const Matrix<ElemType>& tmapTranspose, const Matrix<ElemType>& smap, const Matrix<ElemType>& smapTranspose) const
{
    ...
    vector<ElemType> initialAlpha(nstates, 0);
    initialAlpha[0] = 1.0;
    m_currAlpha->SetValue(nstates, 1, m_deviceId, &initialAlpha[0]);
    m_nextAlpha->Resize(nstates, 1);
    m_alphas->Resize(nstates, nf + 1);
    m_obsp->Resize(nstates, nf + 1);

    vector<ElemType> obsp(nstates * (nf + 1), 0);
    obsp[0] = 1.0;
    obsp[(nf + 1)*nstates - 1] = 1.0;

    m_obsp->SetValue(nstates, nf + 1, m_deviceId, &obsp[0]);
