...
public:
    LatticeFreeMMINode(DEVICEID_TYPE deviceId, const wstring& name)
        : Base(deviceId, name), m_acweight(1.0), m_usePrior(true), m_alignmentWindow(0)
    {
        InitTransitionMatrixes();
    }

    LatticeFreeMMINode(DEVICEID_TYPE deviceId, const wstring& name, const wstring& fstFilePath, const wstring& smapFilePath, const ElemType acweight, bool usePrior, int alignmentWindow)
        : Base(deviceId, name), m_acweight(acweight), m_usePrior(usePrior), m_alignmentWindow(alignmentWindow)
    {
        InitTransitionMatrixes();
    ...
    LatticeFreeMMINode(const ScriptableObjects::IConfigRecordPtr configp)
        : LatticeFreeMMINode(configp->Get(L"deviceId"), L"<placeholder>", configp->Get(L"fstFilePath"), configp->Get(L"smapFilePath"), configp->Get(L"acweight"), configp->Get(L"usePrior"), configp->Get(L"alignmentWindow"))
    {
    ...
    virtual void BackpropToNonLooping(size_t inputIndex) override
    {
        if (inputIndex == 1)
        {
            ...
            //if (m_alignmentWindow == 0)
                //Matrix<ElemType>::AddScaledDifference(Gradient(), *m_posteriorsDen, Input(0)->ValueFor(fr), gradient);
            //else
    ...
    //void PrepareNumeratorMatrixes(const Matrix<ElemType>& labelMatrix);

    double CalculateNumerators(const Matrix<ElemType>& labelMatrix, const size_t nf);

    double ForwardBackwardProcess(const size_t nf, Matrix<ElemType>& posteriors,
        const Matrix<ElemType>& tmap, const Matrix<ElemType>& tmapTranspose, const Matrix<ElemType>& smap, const Matrix<ElemType>& smapTranspose) const;
    ...
    virtual void /*ComputationNodeNonLooping::*/ ForwardPropNonLooping() override
    {
        ...
        auto labelMatrix = Input(0)->MaskedValueFor(fr);
        if (m_acweight != (ElemType)1.0)    // acoustic squashing factor
            (*m_likelihoods) *= m_acweight;

        double logNumerator = 0;
        //if (m_alignmentWindow == 0)
            //logNumerator = Matrix<ElemType>::InnerProductOfMatrices(labelMatrix, *m_likelihoods); // log likelihood on each frame
        ...
        //{
            ...
            //PrepareNumeratorMatrixes(labelMatrix);
            //logNumerator = ForwardBackwardProcess(nf, *m_posteriorsNum, *m_tmapNum, *m_tmapNumTranspose, *m_smapNum, *m_smapNumTranspose);
            logNumerator = CalculateNumerators(labelMatrix, nf);
        ...
        //cout << "numerator score: " << logNumerator << endl;

        double logDenominator = ForwardBackwardProcess(nf, *m_posteriorsDen, *m_tmap, *m_tmapTranspose, *m_smap, *m_smapTranspose);

        // Got the final numbers
        ElemType finalValue = (ElemType)(logDenominator - logNumerator);
        Value().Resize(1, 1);
    ...
    virtual void RequestMatricesBeforeForwardProp(MatrixPool& matrixPool)
    {
        ...
        //{
            ...
            //RequestMatrixFromPool(m_tmapNum, matrixPool);
            //RequestMatrixFromPool(m_smapNum, matrixPool);
            //RequestMatrixFromPool(m_tmapNumTranspose, matrixPool);
            //RequestMatrixFromPool(m_smapNumTranspose, matrixPool);
        //}
    }
    ...
    virtual void ReleaseMatricesAfterForwardProp(MatrixPool& matrixPool)
    {
        ...
        //{
            ...
            //ReleaseMatrixToPool(m_tmapNum, matrixPool);
            //ReleaseMatrixToPool(m_smapNum, matrixPool);
            //ReleaseMatrixToPool(m_tmapNumTranspose, matrixPool);
            //ReleaseMatrixToPool(m_smapNumTranspose, matrixPool);
        //}
    }
    ...
    virtual void DumpNodeInfo(const bool printValues, const bool printMetadata, File& fstream) const override
    {
        if (printMetadata)
        {
            ...
            char str[4096];
            sprintf(str, "acweight=%f usePrior=%d alignmentWindow=%d", this->m_acweight, this->m_usePrior, this->m_alignmentWindow);
            fstream << string(str);
        }
...
protected:
    ...
    //shared_ptr<Matrix<ElemType>> m_tmapNum;
    //shared_ptr<Matrix<ElemType>> m_smapNum;
    //shared_ptr<Matrix<ElemType>> m_tmapNumTranspose;
    //shared_ptr<Matrix<ElemType>> m_smapNumTranspose;
